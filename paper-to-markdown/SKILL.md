...
- `--drop-color` strips `\color{...}` revision markup (e.g. a `\mma`/`\blk` review-comment convention), which otherwise surfaces as literal text mid-sentence. Mention to the user that you dropped it — it is invisible in the PDF and authors often forget it is there.
...
| `escaping-regime` | a real defect — a bare `\command` or `_` escaped into text. Fix the cause, not the symptom |
...
- **Every expression parses.** Run each `$$…$$` (strip `\tag{}` first) and each `$…$` through `katex.renderToString` with `throwOnError`. Watch for accents LaTeX tolerates and KaTeX rejects — `\tilde\mathcal{H}` needs to be `\tilde{\mathcal{H}}` — and for `$a$$b$` from adjacent inline groups, which reads as a display-math opener.
