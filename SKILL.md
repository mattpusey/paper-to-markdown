...
A paper that has no `.bib` at all — a standalone note whose references are typed out as `\begin{thebibliography}` in the source — needs no bibtex and no `.bbl`: that environment is read straight out of the body, numbered in `\bibitem` order exactly as LaTeX numbers it, and emitted as the reference list. `--bbl` is for the bibtex path only, and supplying one anyway is flagged (`bib-source-conflict`) rather than silently preferred.
...
Frontmatter note: `\title`/`\author`/`\email`/`\affiliation`/`\date` sometimes sit *after* `\begin{document}` rather than in the preamble (REVTeX classes do this). Both placements are harvested — the block is read up to `\maketitle`, removed from the body so none of it leaks as raw commands, and emitted as the heading plus author/affiliation lines. Affiliations bind the way REVTeX binds them (each `\affiliation` applies to the run of `\author`s just before it, superscripts numbered in order of appearance, and no superscripts at all when everybody shares one), while authblk's explicit `\author[1]`/`\affil[1]` keys are kept as given. `\email`/`\homepage` addresses follow on a `Contact:` line and a literal `\date` is italicised under them (a `\date{\today}` says nothing at conversion time and is dropped). The abstract, which sits inside that span on a REVTeX paper, is left exactly where it is. Check the resulting author/affiliation pairing against the PDF anyway — it is the one part of the frontmatter that is inferred rather than stated.
...
- `--macro-override NAME=REPLACEMENT` also covers commands that are real LaTeX but never appear as a `\newcommand` in the paper's own source — typically class/package symbols like `\openone` (identity operator, from `revsymb4-1.sty` on REVTeX papers) or `\Zset` from a physics-symbols package. The script only expands macros it found defined in the document, so these pass through untouched and KaTeX will reject them; override with a KaTeX-renderable equivalent (`--macro-override 'openone=\mathbb{1}'`). Two more that turn up often enough to name: `\mathds` (blackboard bold, from `dsfont`) → `--macro-override 'mathds=\mathbb'`, and `\mathsmaller` (from `relsize`, sometimes nested two or three deep for extra shrinkage) → `--macro-override 'mathsmaller='` (empty — KaTeX has no size-shrink equivalent, so this just unwraps the braces and drops the effect).

The script inserts a separating space itself wherever expanding a macro would otherwise glue two letters into a different control word, on either side of the expansion (e.g. a `\newcommand{\ot}{\otimes}` used as `\ot N` used to come out as `\otimesN`; a template like `\langle#2` filled with a letter-starting argument used to come out as `\langlem_c`; and a `\newcommand{\Neg}{N}` written as `\le\Neg(x)` used to come out as the undefined `\leN`) — so this class of KaTeX "Undefined control sequence" failure should be rare now. It can still happen with a macro from `--macro-override` itself if the replacement text doesn't anticipate what follows it; if verification (step 5) turns one up, check whether the surrounding source has a letter running straight into the override.

Two things that are *not* options and must never be read as one: a bracket after a macro (`\Tr[\rho]` is an operator applied to a bracketed argument, not `\Tr` with an option) and a bracket after a theorem-like environment (`\begin{lemma}[branches]` prints the lemma's name). Both are now carried through; only `\smashoperator[r]{...}` and an enumitem option list are consumed. An `enumerate` whose option sets `label=(\alph*)` (or `\roman*`, `\Alph*`, …) keeps those printed labels in the Markdown, because the paper's own prose cites them — "Lemma 2.5(a)", "Remark 3.2(iii)" — and a plain `1.` list breaks every one of those references.

A paper split across files — `\input{preamble}`, `\input{defs.tex}` pulling in the `\newcommand` soup, per-diagram `path.tikz` files loaded via the "tikzit" convention (`\tikzfig{path}`, from `tikzfig.sty`) — is resolved automatically: `\input`/`\include` are inlined recursively, and `\tikzfig`/`\ctikzfig`/`\inlinetikzfig{path}` pulls in `path.tikz` (or `figures/path.tikz`) as a tikzpicture, before anything else runs. Point the script at the *main* file; everything it pulls in is found relative to the file that pulls it in, exactly as pdflatex would. A missing `\input` target is flagged (`input-missing`) rather than silently leaving a gap.
...
| `equation-diagram-split-delimiter` | a `\left`/`\right` pair got split across the fence boundary of a diagram-as-equation (see below) — the surrounding math and the diagram were on either side of an unmatched `\left\{` or similar. KaTeX will reject it; rewrite the math by hand, usually by dropping the `\left`/`\right` sizing (a plain `\{`/`\}` doesn't need a partner) |
...
| `table-derived-number` | a `table` float whose number came from a local counter rather than the `.aux` (no `\label`, or a label the `.aux` doesn't carry) — check it against the PDF |
| `table-image`, `table-unknown`, `table-multi-caption` | a `table` float holding an `\includegraphics`, holding neither a `tabular` nor an image, or carrying several captions (sub-tables) — handle by hand |
| `citet-manual` | `\citet` renders author names via the `.bst`; check the rendering against the PDF |
| `ref-unresolved`, `cite-unresolved`, `no-aux`, `no-bbl` | go back and compile (step 2). `no-bbl` does not fire on a paper whose source carries its own `\begin{thebibliography}` — there is nothing to compile |
| `bib-source-conflict` | the source carries `\begin{thebibliography}` *and* a `.bbl` was found; the source's was used. Check which one the PDF prints — usually the `.bbl` belongs to a different paper in the same folder |
| `no-title` | no `\title` in the preamble *or* before `\maketitle` — the paper genuinely doesn't carry one where it can be found; write the heading by hand from the PDF |
| `frontmatter-dropped` | a frontmatter command with no heading-block equivalent (`\pacs`, `\keywords`, `\preprint`, or a `\thanks`/`\footnote` inside an author name). The content is in the flag detail — restore it by hand wherever it belongs if the PDF prints it |
| `frontmatter-duplicate` | `\title`/`\author` given both in the preamble and after `\begin{document}`; the preamble's was kept and the other is in the flag detail — check which one the PDF shows |
...
- **Cross-references** — every `[n]` resolves to a reference entry, every `Fig. n`/`Table n` in the prose has a caption. `figure` and `table` floats both produce one (`**Figure n:**` / `**Table n:**`). Float numbers are compared as strings, so `\counterwithin` numbering (`Figure 1.1`) and appendix numbering (`A.1`) work; a subfigure reference (`Fig. 4a`) resolves to its parent caption.
...
A `tikzpicture` typeset directly inside an `equation`/`align`/`\[...\]` body — not wrapped in a `figure` float — is a string-diagram identity written *as* an equation ("diagram A = diagram B"), routine in categorical-quantum-mechanics papers. It gets the same node/edge treatment as a figure: the picture becomes a fenced block, whatever math sits around it (an `=`, a `\mapsto`, a trailing condition) stays inline `$...$`, and the result is labelled `**Equation N:**` rather than `$$...$$\tag{N}`, exactly as a figure is labelled instead of tagged. One real limitation: a *multi-row* `align`/`eqnarray` with `&`/`\\` alignment where some rows hold a diagram and others don't is not re-split per row, so the math half of such a row can come out losing its alignment markup (KaTeX will reject a bare `&` outside an `aligned` block) — check these by hand against the PDF; `verify.py`'s KaTeX pass will point at exactly which inline expression fails.
